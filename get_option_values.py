...
def _extract_option_details(container_tag, parent_name=None):
    ...
    if container_tag.name == 'dd':
        ...
        if code_tag_declaration:
            ...
            if len(parts) == 2:
                ...
                # Handle cases where it might just be a name without a type prefix, or malformed
                option_name = declaration_text # Fallback to using the whole text as name
                print(f"Warning: Could not parse type from nested option declaration '{declaration_text}'", file=sys.stderr)
    ...
    if not option_name or not option_type:
        ...
        if container_tag.name == 'dd':
            ...
            print(f"Warning: Could not parse nested option name/type from li tag.", file=sys.stderr)
    ...
    if option_type == 'Boolean':
        ...
        # Look for "Possible values:" list within the current container_tag
        possible_values_heading = container_tag.find(string=re.compile(r'Possible values:'))
