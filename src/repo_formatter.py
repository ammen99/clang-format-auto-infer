...
def run_clang_format_and_count_changes(repo_path, config_string, debug=False, file_sample_percentage=100.0, random_seed=None, worker_id: int = 0):
    """
    ...
    try:
        ...
        try:
            ...
            print(f"Worker {worker_id}: Error listing files in repo: {e}", file=sys.stderr)
            return -1 # Indicate failure

        if not files_to_format:
            if debug:
                 print(f"Worker {worker_id}: No C/C++/Objective-C files found in the repository to format.", file=sys.stderr)
        ...
        if file_sample_percentage < 100.0:
            ...
            if debug:
                print(f"Worker {worker_id}:   Sampling {num_files_to_sample} files ({file_sample_percentage:.1f}%) from {len(files_to_format)} available files.", file=sys.stderr)
        ...
        try:
            ...
            print(f"Worker {worker_id}: Error: clang-format command not found. Please ensure it is installed and in your PATH.", file=sys.stderr)
            # Cannot copy config if clang-format isn't found to even try running it
            sys.exit(1) # Exit immediately as requested
        except subprocess.TimeoutExpired as e:
            print(f"Worker {worker_id}: Warning: clang-format command timed out. Treating as high cost.", file=sys.stderr)
            if debug:
                print(f"Worker {worker_id}: Command: {' '.join(e.cmd)}", file=sys.stderr)
                print(f"Worker {worker_id}: Timeout: {e.timeout}s", file=sys.stderr)
            ...
            if "cannot be used with" in error_output:
                ...
                print(f"Worker {worker_id}: Warning: clang-format reported an invalid configuration ('cannot be used with'). Treating as high cost.", file=sys.stderr)
                if debug:
                    print(f"Worker {worker_id}: Command: {' '.join(e.cmd)}", file=sys.stderr)
                    print(f"Worker {worker_id}: Exit code: {e.returncode}", file=sys.stderr)
                    if e.stdout: print(f"Worker {worker_id}: Stdout:\n{e.stdout}", file=sys.stderr)
                    if e.stderr: print(f"Worker {worker_id}: Stderr:\n{e.stderr}", file=sys.stderr)
                # Return infinity to signify a very bad configuration
                return float('inf')
            else:
                # Other clang-format errors are critical, exit
                print(f"Worker {worker_id}: Error running clang-format with the current configuration:", file=sys.stderr)
                print(f"Worker {worker_id}: Command: {' '.join(e.cmd)}", file=sys.stderr)
                print(f"Worker {worker_id}: Exit code: {e.returncode}", file=sys.stderr)
                if e.stdout:
                     print(f"Worker {worker_id}: Stdout:\n{e.stdout}", file=sys.stderr)
                if e.stderr:
                ...
                try:
                    ...
                    print(f"Worker {worker_id}: Configuration causing the error copied to {error_config_dest} for inspection.", file=sys.stderr)
                except IOError as copy_error:
                    print(f"Worker {worker_id}: Error copying temporary config file to {error_config_dest}: {copy_error}", file=sys.stderr)
        ...
        try:
            ...
            if not insertions_match and not deletions_match and diff_output:
                 ...
                 if debug:
                     print(f"Worker {worker_id}: Warning: Could not parse insertions/deletions from diff output: '{diff_output}'", file=sys.stderr)
            ...
            print(f"Worker {worker_id}: Error running git diff: {e}", file=sys.stderr)
            return -1 # Indicate failure
        ...
        if os.getcwd() != repo_path:
             ...
             try:
                 ...
                 print(f"Worker {worker_id}: Error changing back to repo directory {repo_path} for cleanup: {e}", file=sys.stderr)
        ...
        try:
            ...
            print(f"Worker {worker_id}: Error resetting git repository: {e}", file=sys.stderr)
        ...
        if os.path.exists(temp_config_file):
            try:
                ...
                print(f"Worker {worker_id}: Error removing temporary config file {temp_config_file}: {e}", file=sys.stderr)
        ...
        if os.getcwd() != original_cwd:
             try:
                 ...
                 print(f"Worker {worker_id}: Error changing back to original directory {original_cwd}: {e}", file=sys.stderr)
